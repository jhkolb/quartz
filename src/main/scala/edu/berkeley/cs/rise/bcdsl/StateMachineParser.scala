...
object StateMachineParser extends JavaTokenParsers {
  ...
  def clause: Parser[Expression] = chainl1(atom, booleanOp ^^
      (op => (left: Expression, right: Expression) => LogicalExpression(left, op, right)))

  def logicalExpression: Parser[Expression] = chainl1(clause, comparator ^^
      (op => (left: Expression, right: Expression) => LogicalExpression(left, op, right)))
  ...
  def transition: Parser[Transition] = stateChange ~ opt(authAnnotation) ~ opt(guardAnnotation) ~ opt(transitionBody) ^^
